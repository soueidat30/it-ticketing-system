...
const Icons = {
  ...
  search:
    "M21 21l-4.35-4.35M17 11A6 6 0 115 11a6 6 0 0112 0",
  logout:
    "M9 21H5a2 2 0 01-2-2V5a2 2 0 012-2h4 M16 17l5-5-5-5 M21 12H9",
  chevronL: "M15 18l-6-6 6-6",
  chevronR: "M9 18l6-6-6-6",
  support:
    "M3 18v-6a9 9 0 0118 0v6 M21 19a2 2 0 01-2 2h-1a2 2 0 01-2-2v-3a2 2 0 012-2h3v5z M3 19a2 2 0 002 2h1a2 2 0 002-2v-3a2 2 0 00-2-2H3v5z",
  profile:
...
const NAV_GROUPS = [
  {
    label: "Overview",
    items: [{ label: "Dashboard", to: "/agent/dashboard", icon: "dashboard" }],
  },
  {
    label: "Tickets",
    items: [
      { label: "Assigned Tickets", to: "/agent/assigned-tickets", icon: "assigned", badge: null },
  ...
  {
    label: "Account",
    items: [{ label: "Profile", to: "/agent/profile", icon: "profile" }],
  },
];
...
export default function AgentLayout() {
  ...
  useEffect(() => {
    const loadAssignedCount = async () => {
      ...
      try {
        ...
        const n = Number(data?.stats?.assigned ?? 0);
        setAssignedCount(Number.isFinite(n) ? n : 0);
      } catch {
        // keep 0
      }
    };
  ...
  const navGroupsWithBadges = useMemo(() => {
    return NAV_GROUPS.map((group) => {
      if (group.label !== "Tickets") return group;

      return {
        ...group,
        items: group.items.map((item) => {
          if (item.label !== "Assigned Tickets") return item;
